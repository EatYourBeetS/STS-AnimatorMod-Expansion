...
public class LuciaKonohana extends AnimatorBetaCard
{
    public static final EYBCardData DATA = RegisterSeriesCard(LuciaKonohana.class).SetSkill(-1, CardRarity.UNCOMMON, EYBCardTarget.Normal);
            ...
            for (AbstractPower debuff : corpse.powers)
            {
                for (PowerHelper commonDebuffHelper : GameUtilities.GetCommonDebuffs()) {
                    if (commonDebuffHelper.ID.equals(debuff.ID)) {
                        ...
                        BetaActions.Bottom.ApplyPower(TargetHelper.Enemies(), commonDebuffHelper, powAmount);
                    }
                }
            }
        }
