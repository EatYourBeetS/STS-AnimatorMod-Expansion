...
public class EYBCardTooltip
{
    ...
    private final static FieldInfo<String> _body = BetaJUtils.GetField("BODY", TipHelper.class);
    private final static FieldInfo<String> _header = BetaJUtils.GetField("HEADER", TipHelper.class);
    private final static FieldInfo<ArrayList> _card = BetaJUtils.GetField("card", TipHelper.class);
    private final static FieldInfo<ArrayList> _keywords = BetaJUtils.GetField("KEYWORDS", TipHelper.class);
    private final static FieldInfo<ArrayList> _powerTips = BetaJUtils.GetField("POWER_TIPS", TipHelper.class);
    private final static FieldInfo<Boolean> _renderedTipsThisFrame = BetaJUtils.GetField("renderedTipThisFrame", TipHelper.class);
    ...
    //TODO I added support for rendering tooltips from Blights, but you will need to add a tooltips array to AnimatorBlight as well (or a new generic EYBBlight class)
    private static AnimatorBlight blight;
    private static AbstractCreature creature;
    private static Vector2 genericTipPos = new Vector2(0, 0);

    //TODO I modified the logic for rendering badges to use separate badge background and images
    //TODO You will need to define the Base_Badge texture to make this work
    ...
    public static void QueueTooltips(AnimatorBlight source)
    {
        if (TryRender())
        {
            blight = source;
            GR.UI.AddPostRender(EYBCardTooltip::RenderFromBlight);
        }
    }
    ...
    public static void RenderFromCard(SpriteBatch sb)
    {
        ...
        else
        {
            ...
            if (size > 3f && card.current_y < Settings.HEIGHT * 0.5f && AbstractDungeon.screen != AbstractDungeon.CurrentScreen.CARD_REWARD)
            {
                ...
                y += AbstractCard.IMG_HEIGHT * (0.5f + BetaJUtils.Round(multi * steps, 3));
            }
            else
            {
    ...
    public static void RenderFromBlight(SpriteBatch sb)
    {
        ...
        {
            EYBCardTooltip tip = blight.tips.get(i);
            if (tip.hideDescription == null)
            {
                tip.hideDescription = !StringUtils.isEmpty(tip.id) && GR.Animator.Config.HideTipDescription(tip.id);
            }
    ...
    public float Render(SpriteBatch sb, float x, float y, int index)
    {
        ...
        {
            BetaJUtils.LogWarning(this, "hideDescription was null, why?");
        ...
        if (icon != null)
        {
            ...
            FontHelper.renderFontLeftTopAligned(sb, FontHelper.tipHeaderFont, BetaJUtils.Capitalize(title), x + TEXT_OFFSET_X * 2.5f, y + HEADER_OFFSET_Y, Settings.GOLD_COLOR);
        }
        else
        {
            FontHelper.renderFontLeftTopAligned(sb, FontHelper.tipHeaderFont, BetaJUtils.Capitalize(title), x + TEXT_OFFSET_X, y + HEADER_OFFSET_Y, Settings.GOLD_COLOR);
        }
